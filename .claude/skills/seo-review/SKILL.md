...
- **PR 番号**: 例 `123`（`#` プレフィックス無しの数字で受け取る。シェルで `#` 以降が comment 化される事故を避ける。その PR で変更されたページすべてが対象）
...
- 連続する見出しの間に本文がない（h2 直下に即 h3 が続く等）と読者が文脈を掴みにくく、アクセシビリティ・可読性に影響する（Google の公式ランキング要因ではない）
...
このスキルは **レビュー専門**（単一責任）。`allowed-tools` は `Read, Bash, Grep, Glob` のみで、ファイル編集系ツール（`Edit` / `Write`）は意図的に付与していない。

ユーザーから「直しといて」と指示された場合でも、本スキルからファイルを編集しない。レビュー結果（Critical / Warning と推奨アクション）を出力したうえで、修正は内容に応じて別 Skill に dispatch すること:

- ニュース記事の修正 → `write-blog` または `update-news`
- サービスページの修正 → `add-service-page`
- その他（`src/components/*` / `src/layouts/*` / `src/pages/*` 等） → 手動で対応するか、ユーザーに相談
